...
function User_more_details() {
  ...
  useEffect(function(){
    async function getComplaint(){
      console.log(id)
      try{
        const response = await axios.get(`http://localhost:5000/user/home/complaint/${id}`)
      ...
      {error && <p id="err">{error}</p>}
        ...
        <form>
          ...
          <div className="form_group">
            ...
            <textarea readOnly id="address" className="policeinput_box" value={complaint.address}></textarea>
          </div>
          ...
          <div className="form_group">
            ...
            <textarea readOnly id="vehicle_lost_place" className="policeinput_box" value={complaint.vehicle_lost_place}></textarea>
          </div>

          <div className="form_group">
            <label htmlFor="vehicle_description">VEHICLE DESCRIPTION</label>
            <textarea readOnly id="vehicle_description" className="policeinput_box" value={complaint.vehicle_description}></textarea>
          </div>
          <div className="form_group">
            <label htmlFor="vehicle_description">VEHICLE STATUS</label>
            <textarea readOnly id="vehicle_description" className="policeinput_box" value={complaint.vehicle_found_date?"Your vehicle has been found on " + new Date(complaint.vehicle_found_date).toLocaleTimeString()+". Please contact this: "+ complaint.contact:"Searching in progress ! Please wait"}></textarea>
          </div>
        </form>
      </div>
    </div>
...
export default User_more_details;
